## Черти: Определяне на споделено поведение

(Отличителната) *черта* определя и дава способности, които може да има даден тип,
подобно на други типове със същата (отличителна) черта. Можем да използваме
черти и така да задаваме поведение, присъщо на няколко типа - споделено
поведение. Можем да използваме *граници на черти*, за да определим за някой
обобщен тип, че може да бъде всеки тип, който притежава дадено поведение.


> Забележка: Чертите са подобни на *интерфейсите* (взаимодействие) в други
> езици, но с някои разлики.

### Създаване на черта

Поведението на типа се състои от методите, които можем да извикаме върху този
тип. Различни типове имат едно и също поведение, ако можем да извикаме едни и
същи методи върху тези типове. Определянето на черта е начин да съберем няколко
метода заедно, за да зададем определено поведение, необходимо за постигането на
дадена цел.
...
Например, да кажем, че имаме няколко структури, които съдържат различни видове
и размер текст. Структурата `NewsArticle` има поле „разказ”, а структурата
`Tweet` може да съдържа, най-много 280 знака и някакви метаданни, които указват
дали това е нов `Tweet` дали е повторение, или отговор на друг `Tweet`. 

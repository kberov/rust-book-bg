## Поправими грешки с `Result` 

Повечето грешки не са достатъчно сериозни, за да изискват пълно прекъсване на
програмата. Понякога, когато някоя функиця се провали, грешката може да бъде
обработена и да продължите. Например, ако опитате да отворите файл и действието
е неуспешно, понеже файлът не съществува, може да създадете файла вместо да
спрете изпълнението на програмата.

Recall from [„Обработка на вероятни грешки чрез типа `Result`“][handle_failure]<!-- ignore --> in Chapter 2 that the `Result` enum is

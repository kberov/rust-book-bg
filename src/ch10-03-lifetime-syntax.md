## Проверка на препратки, чрез времетраене

Времетраенето е друг вид обобщение, което вече ползвахме. Вместо да се уверим,
че даден тип се държи както искаме, времетраенето ни уверява, че препратките са
използваеми докато са ни нужни.
...
Има една подробност, която не обсъдихме в раздела [„Препратки и
заемане“][references-and-borrowing]<!-- ignore --> в Глава 4. Тя се състои в
това, че всяка препратка има *времетраене*, което е областта на видимост на
препратката, или къде препратката може да бъде използвана. Като цяло,
времетраенето е подразбрано и изведено (от изходния код), точно както типовете
са изведени.
...
### Предотвратяване на висящи препратки чрез времетраене

Главната цел на времетраенето е да предотврати *висящи препратки*, които
причиняват достъп до данни, които не са предвидени да бъдат достъпни. Да
разгледаме програмата в Пример 10-17. Тя има външна и вътрешна области на
видимост.
...
Във външната област се обявява промелива с име `r` без начална стойност, а във
вътрешната област на видимост се обявява променлива с име `x` и начална
стойност 5. Във вътрешната се опитваме да дадем стойност на `r`, която е
препратка към `x`. След което вътрешната област свършва, а ние се опитваме да
отпечатим стойността на `r`. Този код няма да се компилира, защото стойността,
към която препраща `r` е вече невидима, преди да сме я използвали. Ето
...
В пример 10-19 кодът е поправен, така че да няма висяща препратка и да се компилира успешно.
...
### Правопис за отбелязване на времетраене 
